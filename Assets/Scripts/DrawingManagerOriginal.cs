...
public class DrawingManagerOriginal : MonoBehaviour
{
...
Mathf.Infinity, layerMask);
        ...
        if (Input.GetMouseButtonUp(0) && DrawManager.Instance.GetCurrentInk() > 0)
        {
            EndDrawing();
        }
    ...
    void DrawVisibleLine()
    {
        ...
        if (Vector2.Distance(mousePointer.transform.position - clone.transform.position, newVerticies_[posCount]) > verticesDistance)
        {
            ...
            pathLineRenderer.SetPosition(posCount, mousePointer.transform.position - new Vector3(clone.transform.position.x, clone.transform.position.y, clone.transform.position.z));
            newVerticies_.Add(mousePointer.transform.position - clone.transform.position);
